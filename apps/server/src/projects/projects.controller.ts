...
export class ProjectsController {
  ...
  regenerateFromAgent(@Param('id', ParseUUIDPipe) id: string, @Param('agentKey') agentKey: string) {
    // Validate agentKey shape before it reaches the DB (defense in depth;
    // the service also rejects unknown agents). Keys are lowercase
    // hyphenated identifiers like "requirements-engineering" or "frd".
    if (!/^[a-z0-9][a-z0-9-]*$/.test(agentKey)) {
      throw new BadRequestException(`Invalid agent key: ${agentKey}`);
    }
  ...
  getKnowledgeByAgent(@Param('id', ParseUUIDPipe) id: string, @Param('agentKey') agentKey: string) {
  ...
  @Get(':id/discovery-confirmation')
  getDiscoveryConfirmation(@Param('id', ParseUUIDPipe) id: string) {
    return this.projectsService.getDiscoveryConfirmation(id);
  }

  @Post(':id/discovery-confirmation')
  confirmDiscovery(@Param('id', ParseUUIDPipe) id: string, @Body() dto: ConfirmDiscoveryDto) {
    return this.projectsService.confirmDiscovery(id, dto);
  }

  @Get(':id/run-logs')
  getRunLogs(@Param('id', ParseUUIDPipe) id: string) {
    return this.projectsService.getRunLogs(id);
  }

  @Post(':id/gap-analysis/run')
  runGapAnalysis(@Param('id', ParseUUIDPipe) id: string) {
    return this.projectsService.runGapAnalysis(id);
  }

  @Get(':id/gap-analysis')
  getGapAnalysisHistory(@Param('id', ParseUUIDPipe) id: string) {
    return this.projectsService.getGapAnalysisHistory(id);
  }

  @Post(':id/gap-analysis/proposals/:proposalId/apply')
  applyGapProposal(
    @Param('id', ParseUUIDPipe) id: string,
    @Param('proposalId', ParseUUIDPipe) proposalId: string,
  ) {
    return this.projectsService.applyGapProposal(id, proposalId);
  }

  @Post(':id/gap-analysis/proposals/:proposalId/reject')
  rejectGapProposal(
    @Param('id', ParseUUIDPipe) id: string,
    @Param('proposalId', ParseUUIDPipe) proposalId: string,
  ) {
    return this.projectsService.rejectGapProposal(id, proposalId);
  }

  @Post(':id/gap-analysis/findings/:findingKey/apply')
  applyGapFinding(@Param('id', ParseUUIDPipe) id: string, @Param('findingKey') findingKey: string) {
    return this.projectsService.applyGapFinding(id, findingKey);
  }

  @Post(':id/gap-analysis/proposals/apply-all')
  applyAllGapProposals(@Param('id', ParseUUIDPipe) id: string) {
    return this.projectsService.applyAllGapProposals(id);
  }

  @Post(':id/gap-analysis/stop')
  stopGapAnalysis(@Param('id', ParseUUIDPipe) id: string) {
    return this.projectsService.stopGapAnalysis(id);
  }
  ...
  getVersion(
    ...
    return this.projectsService.getDocumentVersion(id, version, documentType);
  }
