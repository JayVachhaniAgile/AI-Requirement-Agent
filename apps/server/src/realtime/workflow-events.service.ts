...
export class WorkflowEventsService {
  ...
  emitAgentActivity(projectId: string, agentKey: string, phases: AgentActivityPhase[]): void {
  ...
  emitGapAnalysisProgress(
    ...
    this.gateway.emitToProject(projectId, 'gap.analysis.progress', {
      projectId,
      activeRun: payload,
    });
  }
  ...
  emitReasoningTrace(projectId: string, agentKey: string, traces: string[]): void {
  ...
  startActivityPhases(projectId: string, agentKey: string): void {
    ...
    // Safety timeout: auto-clear timer after 10 minutes to prevent memory leaks if agent crashes
    setTimeout(() => {
      this.stopActivityPhases(projectId, agentKey);
    }, 600_000).unref?.();
  }
