...
export class AggregateController {
    ...
    return docs
      ...
      .map(doc => {
        ...
        return {
          ...
          confidence: confidence && !Number.isNaN(confidence)
            ? (confidence <= 10 ? Math.round(confidence * 10) : Math.round(confidence))
            : null,
        };
      });
  }
  ...
  async getAnalytics() {
    ...
    const runningProjects = projects.filter(p => !['COMPLETED', 'CREATED', 'CANCELLED', 'FAILED'].includes(p.status)).length;
    const failedProjects = projects.filter(p => p.status === 'FAILED').length;

    const totalRequirements = allKnowledge.filter(k => k.type === 'FUNCTIONAL_REQUIREMENT').length;
    const validatedRequirements = allKnowledge.filter(k => k.type === 'FUNCTIONAL_REQUIREMENT' && k.status === 'VALIDATED').length;
    const criticalIssues = allIssues.filter(i => i.severity === 'CRITICAL').length;
    const openIssues = allIssues.filter(i => i.status === 'OPEN').length;

    const agentExecutions: Record<string, { count: number; totalDuration: number; successes: number; failures: number }> = {};
    for (const e of executions) {
      const key = e.agentKey ?? 'unknown';
      if (!agentExecutions[key]) agentExecutions[key] = { count: 0, totalDuration: 0, successes: 0, failures: 0 };
      agentExecutions[key].count++;
      if (e.startedAt && e.completedAt) {
        agentExecutions[key].totalDuration += new Date(e.completedAt).getTime() - new Date(e.startedAt).getTime();
      }
    ...
    const pipelineStats = allSteps.reduce((acc, step) => {
    ...
    return {
      ...
      tokenTotals: { inputTokens: totalInputTokens, outputTokens: totalOutputTokens, totalTokens: totalInputTokens + totalOutputTokens },
      totalRequirements,
  ...
  async getWorkflowOverview() {
    ...
    const projectSummaries = projects.map(p => {
      ...
      return {
        ...
        lastActivity: projSteps.length > 0 ? projSteps[projSteps.length - 1].completedAt ?? projSteps[projSteps.length - 1].createdAt : p.createdAt,
      };
    });

    const agentRunHistory = executions.slice(-50).reverse().map(e => ({
      id: e.id,
