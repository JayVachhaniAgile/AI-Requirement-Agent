import { z } from 'zod';
import type { KnowledgeConflict, KnowledgeItemSummary } from './types';
...
export function formatKnowledge(
  ...
  for (const i of filtered) {
    const line = `[${i.type}] ${i.externalId ?? '-'} ${i.title}: ${(i.description ?? '').substring(0, maxDesc)}\n`;
    if (result.length + line.length > maxTotalChars) {
      result += `[truncated: context budget of ${maxTotalChars} chars reached]\n`;
      break;
    }
    result += line;
  }
...
export function withSourceAttribution<K extends Record<string, unknown>>(
  ...
  return items.map((item) => ({
    ...
    confidence: (item.confidence as number) ?? Math.min(
        100,
        Math.round(
          [65,
            item.evidence ? 12 : 0,
...
/**
 * Format `conflicts_with` pairs for validation/debate prompts. Empty when no
 * conflicts exist (keeps existing prompts byte-identical for conflict-free runs).
 */
export function formatConflicts(conflicts?: KnowledgeConflict[]): string {
  if (!conflicts || conflicts.length === 0) return '';
  const lines = conflicts.map(
    (c) =>
      `- [${c.sourceTitle}] (${c.sourceDomain}) conflicts_with [${c.targetTitle}] (${c.targetDomain})` +
      (c.reason ? ` — ${c.reason}` : ''),
  );
  return `Conflicting Items (evaluate consistency and recommend resolution):\n${lines.join('\n')}\n\n`;
}
...
export function safeJsonParse(raw: string): Record<string, unknown> {
  ...
  // All parse strategies exhausted. Throw a structured error so AgentRunnerService
  // correctly categorises this as OUTPUT_PARSE_FAILED and sends the LLM accurate
  // corrective feedback ("your response was not valid JSON") instead of the
  // misleading ARRAY_BELOW_MIN fallback.
  throw new Error('OUTPUT_PARSE_FAILED: LLM response contained no valid JSON object or array');
}
