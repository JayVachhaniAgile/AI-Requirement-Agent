...
import { AGENT_DISPLAY_NAMES } from "@workspace/pipeline-config";
import { apiFetch } from "@/lib/api-fetch";

const AGENT_LABELS = AGENT_DISPLAY_NAMES;
...
function formatDescription(desc: string | null | undefined): string {
  if (!desc) return "";
  // Escape first so injected HTML/scripts in knowledge items cannot render.
  // Bold/code spans are reapplied after escaping using safe pattern matching.
...
export function AgentDetailsModal({
  ...
  const handleRegenerate = () => {
    ...
    apiFetch(`/api/projects/${projectId}/regenerate/${agentKey}`, { method: "POST" })
      .then(async (res) => {
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/projects/${projectId}/knowledge/by-agent/${agentKey}`)
      .then((res) => {
