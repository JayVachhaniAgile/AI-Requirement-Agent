...
export function VersionDiffViewer({ projectId }: { projectId: string }) {
  ...
  if (loading) {
    return <div className="space-y-4"><Skeleton className="h-48 w-full rounded-xl" /><Skeleton className="h-96 w-full rounded-xl" /></div>;
  }
  ...
  if (versions.length === 0) {
    return (
      <div className="text-center p-12 border-2 border-dashed border-border rounded-xl">
        ...
        <p className="text-sm text-muted-foreground mt-1">Versions will appear here after document compilation.</p>
      </div>
    );
  }

  const diffLines = mode === "diff" && currentVersion && compareV
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          {currentVersion?.triggerEvent && (
            <Badge variant="secondary" className="text-xs">{currentVersion.triggerEvent}</Badge>
          )}
        </div>
          ...
          {mode === "view" ? (
            <Button variant="outline" size="sm" onClick={() => {
              ...
              if (!compareVersion && versions.length > 1) {
                ...
                const older = idx >= 0 && idx < versions.length - 1 ? versions[idx + 1] : versions[1] || versions[0];
      ...
      {currentVersion?.changeSummary && (
        <div className="rounded-xl bg-primary/5 border border-primary/20 p-4">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Change Summary</p>
      ...
      {currentVersion && currentVersion.affectedItemIds.length > 0 && (
        <div className="rounded-xl bg-amber-500/5 border border-amber-500/20 p-4">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            This version touches {currentVersion.affectedItemIds.length} requirements, diagrams, or other artifacts.
          </p>
        </div>
      )}
      ...
      <Card className="border-t-4 border-t-primary">
        <CardHeader className="py-3 px-4 border-b bg-muted/20">
          <CardTitle className="text-sm font-semibold">
            {mode === "view" ? `Version ${selectedVersion}` : `Diff: v${compareVersion} → v${selectedVersion}`}
          </CardTitle>
        </CardHeader>
